...
import { getAllBranches } from '../scrape/GetAllBranches';
...
import { slotsRequestsSetup } from '../scrape/SlotsRequestsSetup';
...
router.post(
	...
	async (req: Request, res: Response, next: NextFunction) => {
		...
		// 		const userDataToken = (
		// 			UserCreateAnonymousResponse.data as IUserCreateAnonymous
		// 		).Results.token;
		...
		// 		/*//* ########################################################### */
		// 		const testServiceId = LocationGetServicesResults.Results[0].serviceId;
		...
		// 			SearchAvailableDatesResponse.data as ISearchAvailableDates
		// 		).Results;
		...
		// 			SearchAvailableSlotsResponse.data as ISearchAvailableSlots
		// 		).Results;
		...
		// Puppeteer extract cookies and data.
		const index = getRandomIntInclusive(0, allBranches.length);
		const { cookieBank, htmlToken } = await slotsRequestsSetup(
			allBranches[index]
		);
		console.log('cookieBank : ', cookieBank.getCookies());
		console.log('htmlToken : ', htmlToken);

		while (allBranches.length > 0) {
			try {
				const branch = allBranches.pop();
			} catch (error) {
				console.error({
