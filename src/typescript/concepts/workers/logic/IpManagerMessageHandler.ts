...
> {
	...
	constructor({
		...
		this.verifyDepletedMessage = new VerifyDepletedMessage(requestCounterSetup);
		...
		for (let index = 0; index < this.amountOfUpdaters; index++) {
			const bUpdater = new AppointmentsUpdaterWorker(this.updaterScriptPath, {
				workerData: {
					...
					counterData: this.apiRequestCounterData,
				},
			});
	...
	private resetBatchAndCounters = () => {
		...
		this.verifyDepletedMessage.resetRequestCounter();
		// Reset first-depleted counter.
		this.verifyDepletedMessage.resetDepletedCounter();
