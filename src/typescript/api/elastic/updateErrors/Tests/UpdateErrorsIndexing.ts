...
import { IErrorMapping, UpdateErrorsIndexing } from "../UpdateErrorsIndexing";

console.log("** Test Update Errors Indexing **");
...
export const construct = () => {
	...
	if (!uErrorsIndex)
		throw Error(
			"[construct] Test Failed, UpdateErrorsIndexing is null Or undefined"
		);
...
export const fetchAllErrors = async () => {
	...
	console.log(
		...
		allErrors.data.hits.hits.length
	);
...
export const createErrorIndex = async () => {
	console.log("** (5) BranchServicesIndexing.createBranchIndex **");
	const uErrorsIndex = construct();
	const createSuccess = await uErrorsIndex.createErrorIndex();
