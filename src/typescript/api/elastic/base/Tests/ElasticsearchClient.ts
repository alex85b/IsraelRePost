...
import { BRANCH_INDEX_NAME } from "../../branchServices/constants/Index";
import { buildAllRecordsQuery } from "../../shared/queryBuilders/QueryAllRecordsBuilder";
import { buildDeleteRecordQuery } from "../../shared/queryBuilders/DeleteByQueryBuilder";
import { updateBranchServicesRequest } from "../../branchServices/requests/UpdateBranchServicesRequest";
...
export const getInstance = async () => {
	...
	if (!eClient)
		throw "[getInstance] Test Failed, ElasticsearchClient is null  undefined";
...
export const getIndexMapping = async () => {
	...
	const eClient = await getInstance();
	const response = await eClient.getIndexMapping({
...
export const bulkAdd = async () => {
	...
	const response = await eClient.bulkAdd({
		indexName: TEST_INDEX_NAME,
		bulkedDocuments: bulkDocuments({
			indexName: TEST_INDEX_NAME,
			documentsArray: container,
		}),
	});
	console.log("[addUpdateRecord] : ", JSON.stringify(response, null, 3));
};
