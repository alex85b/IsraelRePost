import { RedisQueueClient } from "../RedisQueueClient";
...
export const enqueue = async () => {
	...
	let enqueuedCount = await redisQueue.enqueue({
		queueName: "testQueue",
		data: enqueueTestA,
	});
	console.log(
		"[enqueue] redisQueue.enqueue enqueueTestA response : ",
		enqueuedCount
	);
	if (enqueuedCount === 0)
		throw Error("[enqueue] Test Failed, redisQueue.enqueue enqueueTestA : 0");

	enqueuedCount = await redisQueue.enqueue({
		queueName: "testQueue",
		data: enqueueTestB,
	});
	console.log(
		"[enqueue] redisQueue.enqueue enqueueTestB response : ",
		enqueuedCount
	);
	if (enqueuedCount === 0)
		throw Error("[enqueue] Test Failed, redisQueue.enqueue enqueueTestA : 0");
