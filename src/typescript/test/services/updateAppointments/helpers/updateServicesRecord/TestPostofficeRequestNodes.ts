...
} from '../../../../../services/updateAppointments/helpers/updateServicesRecord/PostofficeRequestNodes';
...
export const testCreateUserNode = async (
	...
	const errorModelBuilder: IPostofficeUpdateErrorBuilder = new PostofficeUpdateErrorBuilder();
	...
	console.log('[testCreateUserNode] serviceNodes length : ', serviceNodes.length);
	...
	console.log('[testCreateUserNode] serviceNodes toString : ', createUserNode.toString());
...
export const testCreateUserNodeUsingProxy = async (
	...
	console.log('** (1) Test Fetch Branch Services | Test Create User Node Using Proxy **');
	const servicesModelBuilder: IPostofficeBranchServicesBuilder =
		new PostofficeBranchServicesBuilder();
	const errorModelBuilder: IPostofficeUpdateErrorBuilder = new PostofficeUpdateErrorBuilder();
	...
	console.log('[testCreateUserNodeUsingProxy] path to proxy file path : ', proxyFilepath);
	...
	const serviceNodes = await createUserNode.performRequest();
	console.log('[testCreateUserNodeUsingProxy] serviceNodes length : ', serviceNodes.length);
...
export const testFetchServicesNode = async (
	...
	console.log('** (2) Test Fetch Branch Services | Test Fetch Services Node **');
	...
	if (!services[0])
		throw Error('[TestFetchBranchServices][testFetchServicesNode] No Service Node');

	try {
		const service0Dates = await services[0].performRequest();
		console.log('[testFetchServicesNode] service0Dates length : ', service0Dates.length);
...
export const testFetchServicesNodeUsingProxy = async (
	...
	console.log('** (2) Test Fetch Branch Services | Test Fetch Services Node Using Proxy **');
	...
	if (!services[0]) throw Error('[testFetchServicesNodeUsingProxy] No Service Node');
...
export const testFetchDatesNode = async (
	...
	try {
		...
		return { nodes: service0Dates0Times, servicesModelBuilder, errorModelBuilder };
	} finally {
		if (!skipBuild) {
			console.log(
...
export const testFetchDatesNodeUsingProxy = async (
	...
	console.log('** (3) Test Fetch Branch Services | Test Fetch Dates Node Using Proxy **');
	...
	if (!service0Dates[0]) throw Error('[testFetchDatesNodeUsingProxy] No Dates Node');
...
export const testFetchTimesNode = async (skipBuild?: boolean) => {
	...
	if (!service0Dates0Times[0]) throw Error('[testFetchTimesNode] No Times Node');
...
export const testFetchTimesNodeUsingProxy = async (skipBuild?: boolean) => {
	console.log('** (4) Test Fetch Branch Services | Test Fetch Times Node Using Proxy **');
	...
	if (!service0Dates0Times[0]) throw Error('[testFetchTimesNodeUsingProxy] No Times Node');
