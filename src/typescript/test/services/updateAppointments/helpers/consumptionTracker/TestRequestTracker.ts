...
import {
	...
	RequestTracker,
} from '../../../../../services/updateAppointments/helpers/consumptionTracker/RequestTracker';
...
export const testSingleThreadTracking = async () => {
	...
	const memView = new SharedMemoryBuilder().neededCellAmount(2).maxMemoryCellValue(200).build();
	...
	console.log('[testSingleThreadTracking] trackRequest : ', tracker.trackRequest());
	console.log('[testSingleThreadTracking] trackRequest : ', tracker.trackRequest());
	console.log('[testSingleThreadTracking] trackRequest : ', tracker.trackRequest());
	console.log('[testSingleThreadTracking] trackRequest : ', tracker.trackRequest());
	console.log('[testSingleThreadTracking] trackRequest : ', tracker.trackRequest());
	...
	for (let i = 0; i < 10; i++) {
		...
		communicationWrapper.setCallbacks({
			...
			onErrorCallback(error) {
				console.log('[testMultiThreadedTracking] Error : ', JSON.stringify(error, null, 4));
			},
...
export const testLimitReset = () => {
	...
	const { requestTracker } = buildPostOfficePerMinuteLimitTracker({ maximumPerMinute: 2 });
	console.log('[testLimitReset] trackRequest : ', requestTracker.trackRequest());
	console.log('[testLimitReset] trackRequest : ', requestTracker.trackRequest());
	console.log('[testLimitReset] trackRequest : ', requestTracker.trackRequest());
	console.log(
	...
	console.log('[testLimitReset] after-reset trackRequest : ', requestTracker.trackRequest());
	console.log('[testLimitReset] after-reset trackRequest : ', requestTracker.trackRequest());
	console.log('[testLimitReset] after-reset trackRequest : ', requestTracker.trackRequest());
};
