import {
	AtomicArrayWriter,
	IAtomicArrayWriter,
} from '../../../../../services/updateAppointments/helpers/concurrency/AtomicArrayWriter';
import {
	MemoryView,
	SharedMemoryBuilder,
...
export const testSetCellValue = async () => {
	...
	const aWriter: IAtomicArrayWriter = new AtomicArrayWriter({
		memoryView: new SharedMemoryBuilder().maxMemoryCellValue(350).neededCellAmount(2).build(),
...
export const testReplaceExpectedValue = async () => {
	...
	const aWriter: IAtomicArrayWriter = new AtomicArrayWriter({
		memoryView: new SharedMemoryBuilder().maxMemoryCellValue(100).neededCellAmount(3).build(),
		viewParametersExtractor: getMemoryViewParameters,
	});
...
export const testAddToCellValue = async () => {
	...
	const aWriter: IAtomicArrayWriter = new AtomicArrayWriter({
		memoryView: new SharedMemoryBuilder().maxMemoryCellValue(100).neededCellAmount(1).build(),
		viewParametersExtractor: getMemoryViewParameters,
	});
