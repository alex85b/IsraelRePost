...
import {
	...
	HandleStopUpdates,
	IUpdateStarter,
} from '../../../../../services/updateAppointments/workerThreads/appointmentsUpdater/MessageHandlers';
...
import { ConstructServicesRecord } from '../../../../../services/updateAppointments/helpers/updateServicesRecord/ConstructServicesRecord';
...
import { HandlerClass } from '../../../../../services/updateAppointments/helpers/threadCommunication/Handler';
import { IStoppable } from '../../../../../services/updateAppointments/workerThreads/shared/stoppable';
...
if (!workerData)
	throw Error(logMessage.createLogMessage({ subject: 'Invalid workerData - Undefined' }));
console.log(
...
if (typeof workerData !== 'object')
	throw Error(logMessage.createLogMessage({ subject: 'Invalid workerData - Not an object' }));
if (!workerData.memoryView)
	throw Error(logMessage.createLogMessage({ subject: 'Invalid workerData - No memoryView' }));
...
const startUpdates: HandlerClass<IUpdateStarter, AppointmentsUpdatingMessages.StartUpdates> &
...
communicationWrapper.setCallbacks({
	async onMessageCallback(message) {
		console.log(logMessage.createLogMessage({ subject: 'Incoming Message', message }));
		switch (message) {
			...
			case AppointmentsUpdatingMessages.EndUpdater:
				...
				break;
			default:
				throw Error(logMessage.createLogMessage({ subject: 'Unsupported message' }));
		}
	},
});
