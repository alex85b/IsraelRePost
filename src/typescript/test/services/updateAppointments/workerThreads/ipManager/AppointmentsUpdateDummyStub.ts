...
import { IUpdateStarter } from '../../../../../services/updateAppointments/workerThreads/appointmentsUpdater/MessageHandlers';
import { ConstructLogMessage } from '../../../../../shared/classes/ConstructLogMessage';
import { parentPort, workerData, threadId } from 'worker_threads';

const logMessage = new ConstructLogMessage([`AppointmentsUpdateDummyStub ${threadId}`]);
...
communicationWrapper.setCallbacks({
	async onMessageCallback(message) {
		...
		if (message === AppointmentsUpdatingMessages.StartUpdates) {
			...
			communicationWrapper.sendMessage(IpManagerUpdaterMessages.UpdaterDepleted);
		}
