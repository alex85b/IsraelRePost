		...
		IObserveSharedTracking
{
	...
	constructor(buildData: {
		atomicArrayWriter: IAtomicArrayWriter;
		authorizationLimit?: number;
	}) {
		this.pathStack = new PathStack().push("Request Tracker");
		...
		if (memoryCellCount < 2)
			faults.push("IAtomicArrayWriter's memoryCellCount is below 2");
		...
		if (faults.length)
			throw new ServiceError({
				...
				message: "Request track build data arguments are invalid",
				details: {
					faults: faults.join(" | "),
					atomicArrayWriter: buildData.atomicArrayWriter,
					authorizationLimit: buildData.authorizationLimit,
				},
			});
		...
		this.memoryMaxCellValue =
			this.atomicArrayWriter.getMemoryArrayData().memoryMaxCellValue;
	}
	...
	resetTracking(data: {
		...
		const limitSet = this.atomicArrayWriter.setCellValue({
			cell: 1,
			value: data.sharedLimit,
		});
		return valueSet && limitSet;
	}

	trackRequest(): RequestTrackerResponse {
		const incrementResult = this.atomicArrayWriter.addToCellValue({
			cell: 0,
			value: 1,
		});
...
export const buildPostOfficePerMinuteLimitTracker: IBuildPostOfficePerMinuteLimitTracker =
	(args: { maximumPerMinute: number }) => {
