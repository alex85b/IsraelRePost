...
export const repopulateUnprocessedBranchesQueue = async (data: {
	...
	const { processed, unprocessed } = await data.idCodePairRepository.popAllPairs();
	let rePopulate = unprocessed.concat(processed);
	const erroredDuringUpdate = await data.branchesRepository.getAllBranchesIdAndQnomyCodeExcluding(
		rePopulate.map((brach) => brach.branchId)
	);
	if (erroredDuringUpdate.length) rePopulate = erroredDuringUpdate.concat(rePopulate);
